...
# Define inside box coordinates (top left and bottom right) Left Box
...
BR_inside = (int(IM_WIDTH*0.49),int(IM_HEIGHT-5))

# Define outside box coordinates (top left and bottom right) Right Box
#TL_outside = (int(IM_WIDTH*0.46),int(IM_HEIGHT*0.25))
#BR_outside = (int(IM_WIDTH*0.8),int(IM_HEIGHT*.85))

TL_outside = (int(IM_WIDTH*0.50),int(IM_HEIGHT*0.05))
BR_outside = (int(IM_WIDTH*0.95),int(IM_HEIGHT-5))
...
def pet_detector(frame):
    ...
    # Draw boxes defining "right" and "left" locations.
    ...
    cv2.putText(frame,"Left box",(TL_inside[0]+10,TL_inside[1]-10),font,1,(20,255,255),3,cv2.LINE_AA)
    ...
    if (((int(classes[0][0]) == 17) or (int(classes[0][0] == 18) or (int(classes[0][0]) == 1))) and (pause == 0)):
        ...
        if ((x > TL_inside[0]) and (x < BR_inside[0]) and (y > TL_inside[1]) and (y < BR_inside[1]) and (int(classes[0][0] == 17))):
            inside_counter = inside_counter + 1

        # If object is in outside box, increment outside counter variable
        if ((x > TL_outside[0]) and (x < BR_outside[0]) and (y > TL_outside[1]) and (y < BR_outside[1]) and (int(classes[0][0] == 17))):
    ...
    if inside_counter > 10:
        ...
        cv2.imwrite('captured/captured-%s.jpg' % timestr,frame)
        
        song = AudioSegment.from_mp3("sounds/iseeyou.mp3")
        play(song)
     
        message = client.messages.create(
            # body = 'See a' + classes + 'in the left window!',
            body = 'Get out of my candy!',
            from_=twilio_number,
            to=my_number
            )
        inside_counter = 0
    ...
    if outside_counter > 10:
        ...
        message = client.messages.create(
            # body = 'See a' + classes + 'in the right window!',
            body = 'Who is at my desk?',
            from_=twilio_number,
            to=my_number
            )
        inside_counter = 0
    ...
    if pause == 1:
        if detected_inside == True:
            cv2.putText(frame,'Left window!',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(0,0,0),7,cv2.LINE_AA)
            cv2.putText(frame,'Left window!',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(95,176,23),5,cv2.LINE_AA)

        if detected_outside == True:
            cv2.putText(frame,'Right window!',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(0,0,0),7,cv2.LINE_AA)
            cv2.putText(frame,'Right window!',(int(IM_WIDTH*.1),int(IM_HEIGHT*.5)),font,3,(95,176,23),5,cv2.LINE_AA)
